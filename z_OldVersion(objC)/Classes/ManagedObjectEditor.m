...
- (void)viewWillAppear:(BOOL)animated {	
	...
	UIBarButtonItem *currentButton = [[[UIBarButtonItem alloc] initWithTitle:NSLocalizedString(@"Current", @"")       style:UIBarButtonItemStyleBordered  target:self action:@selector(goCurrent:)] autorelease];
...
- (void)goCurrent:(id) sender {  
	
	//BookDetailViewController *second = [[BookDetailViewController alloc] initWithNibName:@"BookDetailView" bundle:nil];
	...
	NSSet *rowSet = [managedObject valueForKey:@"secretIdentity"];
	
	NSString *current = [rowSet heroValueDisplay];
	
	bdvController._text = [current stringByAppendingString:@"/current/"];
...
didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	if ([self isToManyRelationshipSection:[indexPath section]]) {
        ...
        if ([rowSet count] == 0 || [indexPath row] >= [rowSet count]) {
            ...
            controller.title = [NSString stringWithFormat:@"New Camera"];
        }
        else {
